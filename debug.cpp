#include <bits/stdc++.h>

using namespace std;

/*

typedef pair<int, int> ii;
typedef long long ll;
const int oo = 1e9 + 7;
const int modulo = 1e9 + 7;
template<typename T>
inline T sum(const T& a, const T& b) { return T((a % modulo + b % modulo) % modulo); }
template<typename T>
inline T mul(const T& a, const T& b) { return T((1LL * (a % (1LL * modulo)) * (b % (1LL * modulo))) % (1LL * modulo)); }
string to_string(string a) { return "\"" + a + "\""; }
#ifdef __LOCAL_DEBUG
#define debug(args...) { string __out = #args; replace(__out.begin(), __out.end(), ',', ' '); stringstream __out_ss(__out);\
istream_iterator<string> __it(__out_ss); _debug(__it, args); }
void _debug(istream_iterator<string> it) { }
template<typename T, typename... Args>
void _debug(istream_iterator<string> it, T a, Args... args){ cerr << *it << " = " << to_string(a) << '\n'; _debug(++it, args...); }
#else
#define debug(...)
#endif
*/

string to_string(string s) { return "\"" + s + "\""; }

template<typename T>
string to_string(initializer_list<T> a) {
  bool first = true;
  string s = "[";
  for (const auto& val : a) {
    if (first) s += to_string(val);
    else s += ", " + to_string(val);
    first = false;
  }
  s += "]";
  return s;
}

#ifdef __LOCAL_DEBUG
#define debug(args...) debug_process(#args, args);
void debug_out(istream_iterator<string> it) { }

template<typename T, typename... Args>
void debug_out(istream_iterator<string> it, T a, Args... args) {
  cerr << "[DEBUG]: " << *it << " = " << to_string(a) << '\n';
  debug_out(++it, args...);
}

template<typename... Args>
void debug_process(string list_args, Args... args) {
  replace(list_args.begin(), list_args.end(), ',', ' ');
  stringstream ss(list_args);
  istream_iterator<string> it(ss);
  debug_out(it, args...);
}
#else
#define debug(...)
#endif


int main() {
  ios::sync_with_stdio(0);
  cin.tie(0);
  int a = 9, ddddd = 23;
  vector<int> st = { 2, 5, 2, 1};
  set<int> q = { 5, 5, 5, 5, 9, 1 , 7 };
  string we = "werw";
  debug(a, ddddd, we, st);
  return 0;
}
